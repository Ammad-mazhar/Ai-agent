...
async function acceptJob(url, jobData) {
  try {
    await axios.get(url);
    console.log(`✅ SUCCESS: Accepted ${jobData.appliance} in ${jobData.zip}`);
        ...
        msg.on('body', (stream) => {
          simpleParser(stream, async (err, parsed) => {
            ...
            const acceptUrl = links.find(l => {
              ...
              return (low.includes('accept') || low.includes('claim')) && !low.includes('decline') && !low.includes('reject');
            });

            if (zip && ALLOWED_ZIPS.has(zip) && appliance && acceptUrl) {
              if (isPM || jobsAcceptedToday < 4) {
                console.log(`🎯 Valid Job! ${appliance} @ ${zip}. Clicking...`);
