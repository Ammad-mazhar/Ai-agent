...
// ADD ALL YOUR 100+ ZIP CODES HERE
...
const ALLOWED_APPLIANCES = [
  ...
  "stovetop", "range", "cooktop", "dishwasher", "microwave"
];

const imapConfig = {
  user: process.env.EMAIL_USER,
  password: process.env.EMAIL_PASS, // 16-character App Password
  host: 'imap.gmail.com',
  port: 993,
  tls: true,
...
// --- 2. Midnight Reset (Daily 4-Job Limit) ---
setInterval(() => {
  const now = new Date();
...
async function acceptJob(url, jobData) {
  try {
    const response = await axios.get(url);
    console.log(`✅ SUCCESS: Accepted ${jobData.appliance} in ${jobData.zip}. Status: ${response.status}`);
    sendNotification(jobData, "SUCCESS");
  } catch (err) {
    console.error("❌ Link click failed:", err.message);
    sendNotification(jobData, `FAILED: ${err.message}`);
  }
}

// --- 4. The Email Notification (Proof to Boss) ---
async function sendNotification(job, status) {
  ...
  const text = `
        ...
        Type: ${job.isPM ? 'Property Management (Priority)' : 'Standard'}
        Daily Count (Standard): ${jobsAcceptedToday}/4
    `;
      ...
      f.on('message', (msg) => {
        msg.on('body', (stream) => {
          simpleParser(stream, async (err, parsed) => {
            ...
            // Precise Link Finding (Accept NOT Decline)
            const links = parsed.text.match(/https?:\/\/[^\s]+/g) || [];
            const acceptUrl = links.find(l => {
              const low = l.toLowerCase();
              return (low.includes('accept') || low.includes('claim')) &&
            ...
            if (zip && ALLOWED_ZIPS.has(zip) && appliance && acceptUrl) {
              if (isPM || jobsAcceptedToday < 4) {
                console.log(`🎯 Valid Job Found! Zip: ${zip}, App: ${appliance}. Clicking...`);
                await acceptJob(acceptUrl, { zip, appliance, isPM });
                if (!isPM) jobsAcceptedToday++;
              } else {
                console.log("⏳ Standard job limit (4) reached for today. Skipping.");
              }
            }
          });
        });
